...
\author{ZEROUALI Mohammed Amine}
...
d'entree du gradient boosting. Les resultats obtenus montrent un gain moyen de
15 minutes par alerte, avec une calibration des quantiles validee
experimentalement.
\end{abstract}
...
\begin{tabular}{lccc}
\toprule
Aeroport & Alertes (train) & Alertes (test) & Position \\
\midrule
Ajaccio  & 407 & 88 & lon=8.80, lat=41.92 \\
Bastia   & 424 & 126 & lon=9.48, lat=42.55 \\
Biarritz & 486 & 110 & lon=-1.52, lat=43.47 \\
Nantes   & 173 & 41 & lon=-1.61, lat=47.15 \\
Pise     & 623 & 130 & lon=10.40, lat=43.70 \\
\midrule
\textbf{Total} & \textbf{2113} & \textbf{495} & \\
\bottomrule
\end{tabular}
...
Metrique & Valeur \\
\midrule
MAE mediane sur le jeu de validation & 4.30 min \\
Gain moyen (epsilon = 10\%) & 15.4 min \\
Gain median & 16.3 min \\
\% alertes avec gain $>$ 0 & 100\% \\
Theta recommande pour $R < 2\%$ & 0.88 \\
\bottomrule
\end{tabular}
\caption{Resultats du modele LightGBM sur le jeu de validation (442 exemples).}
\end{table}
...
\node[box] (H) at (0,-10.5){predictions.csv (84\,582 lignes)};
...
Statistique & Valeur \\
\midrule
Lignes totales & 84\,582 \\
Aeroports couverts & 5 \\
Alertes couvertes & 316 \\
...
|-- Rapport/
|   |-- Data_Challenge.pdf
|-- main.py                    # pipeline complet + evaluation
|-- README.md
|-- .gitignore
\end{verbatim}
...
# Pipeline complet (entrainement + evaluation + predictions)
python3 main.py

# Generation des predictions uniquement
...
\begin{itemize}
  ...
  \item Inference vectorisee : 35\,000 predictions en $< 30$ secondes
...
\begin{itemize}
  \item Gain estime de 10--15 min par alerte sur $\sim$495 alertes test
  \item Impact direct : reprise plus rapide des operations au sol, reduction des
        temps d'immobilisation des equipages
  \item Reduction du stress operationnel lie a l'incertitude de la fin d'alerte
\end{itemize}
...
l'on extrait le delai optimal $u^*$ sous risque contr\^ole.

Les resultats concrets sont un \textbf{gain moyen de 15.4 minutes} par alerte
avec $\varepsilon = 10\%$, et un fichier \texttt{predictions.csv} de 84\,582
lignes couvrant 316 alertes sur 5 aeroports, pret pour l'evaluation du jury.
